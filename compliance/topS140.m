function topS140(nelx,nely,volfrac,penal,rmin,ft,ftBC,eta,beta,move,pnorm,maxit)
...
betaCnt  = { 2,  16, 50, 1 };                                               % continuation beta
...
nrlo = union(nodeNrs(:,1),nodeNrs(:,end));
...
lcDof = 2 * nodeNrs( 1, : );
F = fsparse( lcDof', 1, -1/numel(lcDof), [ nDof, 1 ] );                     % define load vector
F(lcDof(1), :)   = 0.5*F(lcDof(1), :);
F(lcDof(end), :) = 0.5*F(lcDof(end), :);
% ---------------------------------------------------------- PRE. 4) DAMAGE
[L, nonD, dmg_fac] = deal(20, 5, 1);
% ------------------------------------------------ PRE. 5) STOCHASTIC MODEL
rng('default');                                                             % reset random parameter
com0 = 100;                                                                 % initial guess (scaling)
[y1, y2] = meshgrid( 1:(nelx-L+1) , 1:(nely-L+1-nonD)  ); 
y = [y1(:),y2(:)]';                                                         % integration points
n_disc = size(y,2);                                                         % number of points
X = [randi(nelx-L+1,1,maxit) ; randi(nely-L+1-nonD,1,maxit)];               % sample sequence
maxsmpl = 2000;                                                             % max stored samples
[x_birth,x_ind,leavers] = deal(1:maxsmpl,1:maxsmpl,1);
y_weight = volfrac*sqrt(nEl);                                               % weighting norm
y_diff = pdist2(y',X');
...
while loop < maxit 
    ...
    D = zeros(nely,nelx);
    D( nely+1-(X(2,loop):X(2,loop)+L-1) , X(1,loop):X(1,loop)+L-1 ) = 1;    % damage cell
    x_dmg = max(0,min(1,xPhys-dmg_fac*D(:)));
    sK = ( Emin + x_dmg.^penal * ( E0 - Emin ) );                           % stiffness interpolation
    dsK( act ) = -penal * ( E0 - Emin ) * x_dmg( act ) .^ ( penal - 1 );
    ...
    weights = sum(csw==1:ulim)/n_disc;                                      % integration weights
    if (loop+1) <= maxsmpl
        leavers = loop+1;
    else
